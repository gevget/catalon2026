...
import investorHeroImage from '../assets/1/Инвесторам Главная.webp';
import investorCargoImage from '../assets/1/Грузовые перевозки.webp';
import investorInternationalImage from '../assets/1/ВЭД и международная логистика.webp';
import investorFinanceImage from '../assets/1/Финансовые и сервисные решения.webp';
import investorLogicImage from '../assets/1/Каталон строится как платформа с несколькими точками роста.webp';
import investorPartnersImage from '../assets/1/Нам интересны партнёры, которые способны ускорить развитие Каталон.webp';
import investorStageImage from '../assets/1/Frame 568.webp';
import investorContactImage from '../assets/1/Frame 569.webp';
...
const ecosystem = [
  ['Грузовые перевозки', 'Автомобильные, мультимодальные и контейнерные перевозки с прозрачными условиями, безопасной сделкой и цифровым сопровождением.', 'Грузовые перевозки.png'],
  ['ВЭД и международная логистика', 'Инструменты для организации сложных цепочек поставок, взаимодействия участников и контроля статусов перевозки.', 'ВЭД и международная логистика.png'],
  ['Финансовые и сервисные решения', 'Финансирование, страхование, лизинг, топливо, запчасти, техника и другие услуги для транспортного бизнеса.', 'Финансовые и сервисные решения.png'],
] as const;

const investorImages: Record<string, string> = {
  'Инвесторам Главная.png': investorHeroImage,
  'Грузовые перевозки.png': investorCargoImage,
  'ВЭД и международная логистика.png': investorInternationalImage,
  'Финансовые и сервисные решения.png': investorFinanceImage,
  'Каталон строится как платформа с несколькими точками роста.png': investorLogicImage,
  'Нам интересны партнёры, которые способны ускорить развитие Каталон.png': investorPartnersImage,
  'Frame 568.png': investorStageImage,
  'Frame 569.png': investorContactImage,
};
function InvestorArt({ name, alt, className = '' }: { name: string; alt: string; className?: string }) {
  return <img src={investorImages[name]} alt={alt} loading={name === 'Инвесторам Главная.png' ? 'eager' : 'lazy'} fetchPriority={name === 'Инвесторам Главная.png' ? 'high' : 'auto'} decoding="async" className={`block h-auto w-full object-contain ${className}`} />;
}
...
const investmentDirections = [
  ...
  ['Подключение участников', 'Перевозчики, грузовладельцы, экспедиторы и партнёры экосистемы.', Handshake],
...
const faqs = [
  ...
  ['Можно ли участвовать не только капиталом?', 'Да. Для Каталон важны также отраслевые знания, доступ к клиентам, инфраструктура, технологии, каналы продаж и помощь в формировании команды.'],
      ...
      <section id="investor-overview" className="relative overflow-hidden bg-[#24004C] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        ...
        <div className="relative mx-auto grid max-w-[1208px] items-center gap-12 lg:grid-cols-[1fr_0.9fr]">
          <div>
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-[#D8CFE5]">Каталон развивает комплексную платформу для грузовых перевозок, ВЭД и сопутствующих сервисов.</p>
            <p className="mt-4 max-w-xl leading-7 text-white/65">Мы объединяем логистику, автоматизацию, финансовые и сервисные решения в единую экосистему для бизнеса. Сейчас Каталон развивается на средства основателей и открыт к диалогу со стратегическими партнёрами и инвесторами.</p>
          ...
          <div className="investor-hero-art overflow-hidden bg-transparent"><InvestorArt name="Инвесторам Главная.png" alt="Цифровая инфраструктура грузовых перевозок Каталон" /></div>
        </div>
      </section>

      <section id="investor-ecosystem" className="bg-[#440D84] px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto max-w-[1208px]"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#A0FF00]">ЕДИНАЯ ЭКОСИСТЕМА</p><h2 className="mt-4 max-w-3xl text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Что создаёт Каталон</h2><p className="mt-5 max-w-2xl leading-7 text-white/65">Мы создаём инфраструктуру вокруг реальных задач транспортного бизнеса — от перевозки до сопутствующих сервисов.</p><div className="mt-12 grid gap-4 lg:grid-cols-3">{ecosystem.map(([title, text, image]) => <article key={title} className="group overflow-hidden rounded-2xl border border-white/15 bg-[#2A0751]/45 p-4 transition hover:-translate-y-1 hover:border-[#A0FF00]/60"><InvestorArt name={image} alt={title} className="aspect-[16/10] object-cover" /><h3 className="mt-6 text-xl font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-white/65">{text}</p></article>)}</div></div></section>

      <section id="investor-approach" className="bg-[#2A0751] px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto max-w-[1208px]"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#BA9AF0]">НАШ ПОДХОД</p><h2 className="mt-4 max-w-3xl text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Каталон объединяет разрозненные процессы в понятную цифровую систему</h2><div className="mt-12 grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">{approach.map(([title, text, Icon]) => <article key={title} className="flex gap-4 border-t border-white/15 pt-5"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#561A9D] text-[#A0FF00]"><Icon className="h-5 w-5" /></div><div><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-white/60">{text}</p></div></article>)}</div></div></section>

      <section id="investor-logic" className="bg-[#B3FB01] px-5 py-20 text-[#1A1A1A] sm:px-8 lg:px-10 lg:py-24"><div className="mx-auto grid max-w-[1208px] items-start gap-10 lg:grid-cols-[0.85fr_1.15fr]"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#440D84]">ИНВЕСТИЦИОННАЯ ЛОГИКА</p><h2 className="mt-4 max-w-3xl text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Каталон строится как платформа с несколькими точками роста</h2><p className="mt-5 max-w-2xl leading-7">Ниже — логика развития платформы без подмены её конкретными финансовыми прогнозами или обещаниями доходности.</p><div className="mt-10 grid gap-3">{logic.map(([title]) => <div key={title} className="rounded-xl border border-[#440D84]/20 bg-white/25 px-5 py-4 font-bold text-[#440D84]">{title}</div>)}</div></div><InvestorArt name="Каталон строится как платформа с несколькими точками роста.png" alt="Несколько направлений роста платформы Каталон" /></div></section>
