...
export default function PartnersPage() {
  ...
  return <div className="partners-page">
    ...
    <main>
      ...
      <footer id="footer-early" className="partners-footer partners-footer-early"><div className="partners-container"><div className="partners-footer-top"><div><span className="partners-footer-brand">CATALON</span><p>Catalon объединяет участников, сервисы и технологии вокруг реальной перевозки.</p></div><div className="partners-footer-column"><b>Партнёрство</b><a href="#formats">Подключить сервис</a><a href="#formats">Совместный продукт</a><a href="#integrations">Интеграция</a></div><div className="partners-footer-column"><b>Проекты</b><a href="#special-projects">Специальный проект</a><a href="#selection">Требования</a><a href="#form">Контакты</a></div></div><div className="partners-footer-bottom"><span>© Catalon</span><span>Партнёрская экосистема</span></div></div></footer>

      <section id="process" className="partners-process">
        <div className="partners-container"><div className="partners-section-intro"><div><p className="partners-kicker">НЕ ПРОСТО ОХВАТ — БИЗНЕС-КОНТЕКСТ</p><h2>Ваш продукт становится частью конкретной задачи пользователя</h2></div><p>Catalon связывает партнёрское предложение с контекстом перевозки: от подготовки рейса и финансирования до решения вопроса в пути или развития автопарка.</p></div>
          <div className="partners-process-panel"><div className="partners-process-lines">{processRoles.map(({ icon: IconComponent, role, title, text, items }, index) => <article key={role} className="partners-process-row"><div className="partners-role-mark"><IconComponent size={20} strokeWidth={1.6} /><span>0{index + 1}</span></div><div className="partners-role-copy"><p className="partners-role-label">{role}</p><h3>{title}</h3><p>{text}</p><div className="partners-role-items">{items.map((item) => <span key={item}>{item}</span>)}</div></div><div className="partners-process-connector"><i /><span>Продукт партнёра</span><ArrowRight size={17} /></div></article>)}</div><div className="partners-process-caption"><ShieldCheck size={18} />Партнёрский продукт появляется там, где помогает выполнить конкретную задачу перевозки.</div></div>
        </div>
      </section>
