  ...
  return <header className="unified-header sticky top-0 z-50 bg-[#440D84] text-white shadow-[0_8px_30px_rgba(36,8,67,0.15)]">
    <div className="mx-auto flex h-[48px] max-w-[1440px] items-center justify-between px-5 lg:px-8">
      ...
      <nav className="hidden items-center gap-6 text-sm font-semibold lg:flex">
        ...
        {spotlightLinks.map(([label, href]) => <a key={label} href={href} className="whitespace-nowrap font-bold text-[#B7FF2A] transition hover:text-white">{label}</a>)}
    ...
    {open && <nav className="grid gap-1 border-t border-white/10 bg-[#440D84] px-5 pb-5 lg:hidden">
      ...
      {spotlightLinks.map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)} className="border-t border-white/10 py-3 text-sm font-bold text-[#B7FF2A]">{label}</a>)}
