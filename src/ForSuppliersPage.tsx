      ...
      <main>
        ...
        <section id="supplier-value" className="bg-[#F0E8FA] px-5 py-20 sm:px-8 lg:px-10 lg:py-24">
          <div className="mx-auto max-w-[1208px]">
            ...
            <p className="mt-6 max-w-2xl leading-7 text-[#675F6F]">Аудитория Catalon сталкивается с задачами до рейса, перед отправлением, в пути, после выгрузки и при развитии бизнеса. Поставщик подключается к конкретному сценарию, а не просто размещается в каталоге.</p>
        ...
        <section id="supplier-form" className="bg-[#F7F7F5] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="mx-auto grid max-w-[1208px] items-start gap-12 lg:grid-cols-[1fr_0.9fr]">
            <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#561A9D]">НАЧАТЬ СОТРУДНИЧЕСТВО</p><h2 className="mt-4 text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Если подходящего сервиса ещё нет — предложите его</h2><p className="mt-6 max-w-xl leading-7 text-[#8496AD]">Оставьте контакты и опишите продукт. Мы проверим, в каком сценарии он будет полезен аудитории Catalon, и предложим формат запуска.</p><div className="mt-8 flex items-center gap-3 text-sm text-[#440D84]"><Mail className="h-5 w-5" /> Открыты к новым решениям</div></div>
