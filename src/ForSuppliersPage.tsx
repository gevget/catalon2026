...
import supplierHeroImage from '../assets/1/Frame 566.webp';
import supplierDemandImage from '../assets/1/image 413.webp';
import supplierPartnershipImage from '../assets/1/supplier-partnership.webp';
...
const steps = [
  ...
  ['04', 'Запуск', 'Согласуем детали и подготовим продукт к появлению в экосистеме Каталон.', PackageCheck],
] as const;

const demandStages = [
  ['До рейса', 'Финансирование, лизинг и страхование помогают подготовить перевозку.', CircleDollarSign],
  ['Перед отправлением', 'Топливо, техника и документы поддерживают запуск маршрута.', Truck],
  ['В пути', 'Связь, защита и сервисы помогают решать задачи по ходу перевозки.', ShieldCheck],
  ['После выгрузки', 'Расчёты, обслуживание и повторные решения возвращают клиента в экосистему.', FileText],
  ['Развитие', 'Поставщик может масштабировать продукт вместе с растущими перевозками.', Network],
] as const;
      ...
      <main id="main-content">
        <section id="supplier-overview" className="bg-[#F6F5F4] px-5 py-20 sm:px-8 lg:px-10 lg:py-28" style={{ backgroundColor: '#F6F5F4' }}>
          <div className="mx-auto grid max-w-[1208px] items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]">
            <div>
              ...
              <p className="mt-7 max-w-xl text-lg leading-8 text-[#8496AD]">Подключайте банковские, страховые, лизинговые и сервисные продукты к реальным бизнес-сценариям перевозки — там, где у аудитории возникает потребность.</p>
              <div className="mt-9 flex flex-wrap gap-3">
                ...
                <a href="#supplier-directions" className="inline-flex items-center gap-2 rounded-lg border border-[#440D84] px-6 py-3.5 text-sm font-bold text-[#440D84] transition hover:bg-white">Узнать о Каталон</a>
              </div>
        ...
        <section id="supplier-value" className="bg-[#EADCFA] px-5 py-20 sm:px-8 lg:px-10 lg:py-24" style={{ backgroundColor: '#EADCFA' }}>
          <div className="mx-auto max-w-[1208px]">
            ...
            <p className="mt-6 max-w-2xl leading-7 text-[#675F6F]">Аудитория Каталон сталкивается с задачами до рейса, перед отправлением, в пути, после выгрузки и при развитии бизнеса. Поставщик подключается к конкретному сценарию, а не просто размещается в каталоге.</p>
        ...
        <section id="supplier-process" className="bg-[#F7F7F5] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="mx-auto max-w-[1208px]">
            ...
            <h2 className="mt-4 max-w-3xl text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Подключиться к Каталон можно поэтапно</h2>
        ...
        <section id="supplier-form" className="bg-[#F7F7F5] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="mx-auto grid max-w-[1208px] items-start gap-12 lg:grid-cols-[1fr_0.9fr]">
            <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#561A9D]">НАЧАТЬ СОТРУДНИЧЕСТВО</p><h2 className="mt-4 text-4xl font-extrabold tracking-[-0.04em] sm:text-5xl">Если подходящего сервиса ещё нет — предложите его</h2><p className="mt-6 max-w-xl leading-7 text-[#8496AD]">Оставьте контакты и опишите продукт. Мы проверим, в каком сценарии он будет полезен аудитории Каталон, и предложим формат запуска.</p><div className="mt-8 flex items-center gap-3 text-sm text-[#440D84]"><Mail className="h-5 w-5" /> Открыты к новым решениям</div></div>
            <form onSubmit={handleSubmit} className="rounded-2xl border border-[#DBDDE5] bg-white p-6 sm:p-8">
              {submitted ? <div className="py-12 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[#A0FF00] text-[#440D84]"><Check className="h-7 w-7" /></div><h3 className="mt-5 text-2xl font-bold">Заявка получена</h3><p className="mt-3 text-sm leading-6 text-[#8496AD]">Спасибо! Мы изучим информацию и свяжемся с вами для обсуждения возможного сотрудничества.</p></div> : <><div className="grid gap-5 sm:grid-cols-2"><label className="text-sm font-semibold">ФИО<input required name="name" autoComplete="name" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="text-sm font-semibold">Компания<input required name="company" autoComplete="organization" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="text-sm font-semibold">ИНН<input name="inn" inputMode="numeric" autoComplete="off" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="text-sm font-semibold">Телефон (необязательно)<input type="tel" name="phone" autoComplete="tel" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="text-sm font-semibold">E-mail<input required type="email" name="email" autoComplete="email" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="text-sm font-semibold">Сфера деятельности<input required name="industry" autoComplete="organization-title" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label></div><label className="mt-5 block text-sm font-semibold">Ссылка на сайт<input type="url" name="website" autoComplete="url" className="mt-2 h-12 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 outline-none focus:border-[#A0FF00]" /></label><label className="mt-5 block text-sm font-semibold">Комментарий о продукте или предложении<textarea required name="comment" rows={4} className="mt-2 w-full rounded-lg border border-[#B0BDCE] bg-[#F7F7F5] px-4 py-3 outline-none focus:border-[#A0FF00]" /></label><button type="submit" className="mt-6 h-12 w-full rounded-lg bg-[#A0FF00] px-5 text-sm font-bold text-[#1A1A1A] transition hover:bg-[#8FE500]">Отправить заявку</button></>}
            </form>
          </div>
        </section>
      </main>
