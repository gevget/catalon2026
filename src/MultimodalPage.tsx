...
import multimodalHeroImage from '../assets/1/multimodal-hero.webp';
import geographyMapImage from '../assets/1/multimodal-geography-map-catalon-regions.webp';
import cargoTypesImage from '../assets/1/multimodal-cargo-types.webp';
import partnersNetworkImage from '../assets/1/multimodal-partners-network.webp';
import multimodalCustomerImage from '../assets/1/multimodal-customers.webp';
import multimodalTransportPartnerImage from '../assets/1/multimodal-transport-partners.webp';
import stageControlImage from '../assets/1/Вся операционная работа в одной системе.webp';
import documentsStageImage from '../assets/1/Документы.webp';
import multimodalDocumentsImage from '../assets/1/Документы связаны с этапами.webp';
import roadFreightSolutionImage from '../assets/1/Нужна автомобильная перевозка по России_.webp';
...
function AssetPlaceholder({ name, dark = false }: { name: string; dark?: boolean }) {
  ...
  if (suppliedImage) return <img src={suppliedImage} alt={name === 'multimodal-documents.png' ? 'Документы связаны с этапами перевозки' : 'Автомобильная перевозка по России'} loading="lazy" decoding="async" className="block h-auto w-full rounded-2xl object-contain" />;
  return <div className={`grid min-h-64 place-items-center rounded-[28px] border p-8 text-center ${dark ? 'border-white/15 bg-white/5 text-white/45' : 'border-[#DDD3EA] bg-[#F3ECFA] text-[#776887]'}`} role="img" aria-label={name}><Container className="h-12 w-12" strokeWidth={1.2} aria-hidden="true" /><span className="sr-only">{name}</span></div>;
}
...
const audience = [
  ...
  { icon: Handshake, title: 'Транспортным партнёрам', text: 'Для крупных контейнерных линий и терминальных компаний — подключение к цифровому контуру маршрута и обмен актуальными статусами.', points: [[Link2, 'Подключение к маршруту'], [RadioTower, 'Передача статусов'], [FileCheck2, 'Данные этапа в карточке']], file: 'multimodal-partners.png' },
];

const portalFeatures: [LucideIcon, string, string][] = [
  [PanelTop, 'Карточка маршрута', 'Заявка, тариф, этапы и участники собраны в одном контексте.'],
  [Users, 'Участники и роли', 'Ответственные по этапам видны заказчику и грузовладельцу.'],
  [Link2, 'Передача этапа', 'Фиксация передачи груза и следующего ответственного.'],
  ...
  [SlidersHorizontal, 'Расчёт и аналитика', 'Тариф, сроки и результаты маршрута в понятном виде.'],
...
const faq = [
  ...
  ['Каталон сам выполняет все этапы перевозки?', 'Каталон работает как цифровой портал для заказчиков и транспортных партнёров. Конкретные этапы выполняются подключёнными участниками маршрута, а статусы и документы сохраняются в едином контуре.'],
  ...
  ['Кто координирует передачу между этапами?', 'Координация зависит от выбранной схемы маршрута и его участников. Каталон связывает этапы, статусы, документы и ответственных в одной карточке.'],
  ['Есть ли безопасная сделка?', 'Каталон помогает фиксировать условия, участников, документы и важные события внутри платформы, снижая риски для сторон.'],
  ...
  ['Входит ли таможенное оформление?', 'Таможенное оформление развивается как отдельное решение Каталон и не является автоматически включённой частью контейнерного маршрута.'],
  ...
  ['Кому подходит решение?', 'В первую очередь заказчикам и грузовладельцам, которым нужен контроль сложного маршрута. Также решение подходит крупным контейнерным линиям и терминальным транспортным партнёрам, подключающимся к цифровому контуру.'],
] as const;
...
export default function MultimodalPage() {
  ...
  useEffect(() => {
    ...
    document.title = 'Мультимодальные и контейнерные перевозки через Каталон';
    if (description) description.content = 'Управляйте контейнерными и мультимодальными маршрутами, этапами, участниками, документами и безопасной сделкой через единый портал Каталон.';
    const blockTags: Array<[string, string]> = [
      ...
      ['mm-process', 'Как Каталон собирает маршрут'],
      ['stage-control', 'Контроль исполнения по этапам'],
      ['mm-portal-features', 'Возможности портала'],
      ['mm-operator', 'Рабочее место экспедитора маршрута'],
    ...
    const operatorSection = Array.from(document.querySelectorAll<HTMLElement>('main section')).find((section) => section.textContent?.includes('Рабочее место экспедитора маршрута'));
    ...
    if (routeSection && !document.getElementById('mm-digital-cabinet')) {
      ...
      if (geographySection) {
        ...
        map.alt = 'Карта мультимодальных маршрутов Каталон';
      ...
      if (audienceSection) {
        ...
        const audienceCopy = [
          ...
          'Экспедитор координирует участников, документы и события, чтобы заказчик видел полную картину и мог быстро реагировать на изменения.',
        ];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white text-[#19131F]">
      <UnifiedHeader />
      <ProductAnchorNav items={[["Обзор", "#mm-top"], ["Маршрут", "#route"], ["Участники", "#mm-audience"], ["Этапы", "#mm-process"], ["Контроль", "#stage-control"], ["Документы и сделка", "#documents"], ["Портал", "#mm-portal-features"], ["FAQ", "#faq"]]} />
      ...
      <main id="main-content">
        <section className="bg-[#3D0B78] text-white" id="mm-top" style={{ backgroundColor: '#3D0B78' }}>
          <div className="mx-auto max-w-[1440px] px-4 pb-20 pt-6 sm:px-6 lg:px-10 lg:pb-24">
            ...
            <div className="mt-10 grid min-h-[650px] items-center gap-12 lg:grid-cols-[1.08fr_0.92fr]">
              <div><div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/8 px-4 py-2 text-xs font-bold"><span className="h-2 w-2 rounded-full bg-[#A0FF00]" />Активное решение Каталон</div><h1 className="mt-7 text-4xl font-bold sm:text-5xl lg:text-6xl">Мультимодальные перевозки — <span className="text-[#BA9AF0]">в одном цифровом маршруте</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-white/72">Объединяйте автомобильные, железнодорожные, морские и терминальные этапы. Управляйте заявкой, участниками, документами и статусами через единый портал Каталон.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href="https://container.catalon.ru/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#A0FF00] px-7 py-4 text-sm font-bold text-[#2E075A]">Создать заявку <ArrowRight className="h-4 w-4" /></a><a href="#route" className="inline-flex items-center justify-center rounded-full border border-white/30 px-7 py-4 text-sm font-bold">Посмотреть, как работает</a></div><div className="mt-9 flex flex-wrap gap-2">{['Контейнеры', 'Комбинированные маршруты', 'Контроль этапов', 'Документы онлайн', 'Статусы маршрута'].map((tag) => <span key={tag} className="rounded-full border border-white/15 px-3 py-2 text-xs text-white/70">{tag}</span>)}</div></div>
              <img src={multimodalHeroImage} alt="Мультимодальный маршрут Каталон" fetchPriority="high" decoding="async" className="w-full object-contain" />
            </div>
          </div>
        </section>

        <section className="border-b border-[#E5DDF0] bg-[#F6F1FB]"><div className="mx-auto grid max-w-[1440px] grid-cols-2 gap-px bg-[#DDD1EB] px-4 sm:px-6 md:grid-cols-5 lg:px-10">{[[ClipboardList, 'Единая заявка'], [Network, 'Несколько видов транспорта'], [ListChecks, 'Контроль этапов'], [FileCheck2, 'Документы онлайн'], [RadioTower, 'Статусы маршрута']].map(([Icon, label]) => { const ItemIcon = Icon as LucideIcon; return <div key={label as string} className="flex items-center gap-3 bg-[#F6F1FB] px-3 py-6 text-sm font-semibold"><ItemIcon className="h-5 w-5 shrink-0 text-[#561A9D]" />{label as string}</div>; })}</div></section>

        <section className="mx-auto grid max-w-[1440px] items-center gap-12 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:px-10 lg:py-28"><div><SectionIntro eyebrow="Не просто перемещение контейнера" title="Один маршрут связывает несколько самостоятельных операций" text="Мультимодальная перевозка — это последовательность этапов, исполнителей и передач ответственности. Каталон собирает их вокруг одной заявки, чтобы маршрут оставался управляемым от первой до последней мили." /><div className="mt-8 grid gap-3 sm:grid-cols-2">{[[Truck, 'Автомобильный этап'], [RailSymbol, 'Железнодорожный этап'], [Ship, 'Морской этап'], [Warehouse, 'Терминальные операции']].map(([Icon, label]) => { const ModeIcon = Icon as LucideIcon; return <div key={label as string} className="flex items-center gap-3 border-b border-[#DDD3E7] py-4 text-sm font-bold"><ModeIcon className="h-5 w-5 text-[#7F42E1]" />{label as string}</div>; })}</div></div><AssetPlaceholder name="multimodal-definition.png" /></section>
        ...
        <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-10 lg:py-28"><SectionIntro eyebrow="Сборка маршрута" title="Один маршрут — несколько понятных этапов" text="Заказчик видит, как заявка превращается в последовательность действий: от расчёта и согласования до передачи и закрытия перевозки." /><ol className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-6">{['Заявка', 'Маршрут', 'Исполнители', 'Этапы', 'Документы', 'Завершение'].map((title, index) => <li key={title} className="rounded-[22px] border border-[#DDD3E7] bg-white p-5 transition hover:-translate-y-1 hover:border-[#7F42E1] hover:shadow-[0_16px_30px_rgba(68,13,132,0.12)]"><div className="grid h-12 w-12 rotate-45 place-items-center rounded-[10px] bg-[#F1E8FA] text-[#7F42E1]"><span className="-rotate-45 text-xs font-bold">0{index + 1}</span></div><h3 className="mt-7 text-base font-bold">{title}</h3><p className="mt-2 text-xs leading-5 text-[#675F6F]">{['Задать груз и направление.', 'Собрать автомобильный, терминальный, железнодорожный или морской этап.', 'Определить участников каждого перехода.', 'Видеть статус и ответственного по каждому этапу.', 'Хранить подтверждения и закрывающие файлы.', 'Зафиксировать результат и историю маршрута.'][index]}</p></li>)}</ol></section>

        <section className="bg-[#440D84] py-20 text-white lg:py-28"><div className="mx-auto grid max-w-[1440px] gap-12 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-10"><div><SectionIntro eyebrow="Контроль исполнения" title="Статус каждого этапа — в одной картине маршрута" text="Заказчик и грузовладелец видят события, документы, сроки и передачу ответственности без необходимости собирать статусы по звонкам и чатам." light /><div className="mt-9 space-y-3">{['Первая миля', 'Терминал или железная дорога', 'Морская линия', 'Последняя миля', 'Маршрут завершён'].map((status, index) => <div key={status} className="flex items-center justify-between border-b border-white/15 py-3"><span className="text-sm font-semibold">{status}</span><span className={`text-xs font-bold ${index < 3 ? 'text-[#A0FF00]' : 'text-white/45'}`}>{index < 3 ? 'Статус виден' : 'Следующий этап'}</span></div>)}</div></div><img src={stageControlImage} alt="Контроль этапов мультимодального маршрута" className="w-full object-contain" /></div></section>
        ...
        <section className="hidden bg-[#F1E8FA] py-20 lg:py-28" aria-hidden="true"><div className="mx-auto grid max-w-[1440px] items-center gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:px-10"><AssetPlaceholder name="operator-workplace.png" /><div><SectionIntro eyebrow="Ключевая роль" title="Рабочее место экспедитора маршрута" text="Экспедиторская координация не является отдельным предложением мультимодального решения." /></div></div></section>
