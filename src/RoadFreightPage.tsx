...
import roadFreightHeroImage from '../assets/1/road-freight-hero.webp';
import roadFreightPanelImage from '../assets/1/road-freight-panel.webp';
import roadFreightAnalyticsImage from '../assets/1/road-freight-analytics.webp';
import safeDealAsset from '../assets/1/Безопасная сделка.webp';
import businessToolsAsset from '../assets/1/Возможности больших компаний — для малого и среднего бизнеса.webp';
import multimodalAsset from '../assets/1/Мультимодальные и контейнерные перевозки.webp';
import financeAsset from '../assets/1/Финансирование под конкретный рейс.webp';
import portalOverviewAsset from '../assets/1/Посмотрите, как устроена работа в Catalon.webp';
import earningAsset from '../assets/1/Зарабатывать.webp';
import savingAsset from '../assets/1/Экономить.webp';
import customerAsset from '../assets/1/Перевозка под контролем, а не в переписках.webp';
import carrierAsset from '../assets/1/Больше подходящих рейсов без ручного поиска.webp';
import operatorAsset from '../assets/1/Вся операционная работа в одной системе.webp';
import tripsAsset from '../assets/1/Рейсы.webp';
import documentsAsset from '../assets/1/Документы.webp';
import financesAsset from '../assets/1/Финансы.webp';
import servicesAsset from '../assets/1/Сервисы.webp';
import supportAsset from '../assets/1/Поддержка.webp';
...
function AssetPlaceholder({ name, dark = false }: { name: string; dark?: boolean }) {
  ...
  if (image) return <img src={image} alt="" loading="lazy" decoding="async" className="block h-auto w-full rounded-[16px] object-contain" />;
  return <div className={`grid min-h-44 place-items-center border ${dark ? 'border-white/15 bg-white/8 text-white/45' : 'border-[#DED9E3] bg-[#F7F6F3] text-[#9B91A6]'}`} role="img" aria-label={name}><PackageSearch className="h-9 w-9 opacity-55" aria-hidden="true" /><span className="sr-only">{name}</span></div>;
}
...
const onlineCards = [
  ...
  ['Поддержка', 'Помощь экспедитора на каждом этапе.', supportAsset, Headphones],
...
const faq = [
  ...
  ['Каталон — перевозчик или биржа грузов?', 'Каталон — цифровая платформа для совместной работы заказчиков, перевозчиков и экспедиторов. Она объединяет поиск рейса, проведение сделки, документы, расчёты и сервисы.'],
  ...
  ['Есть ли помощь при подключении?', 'Да. Команда Каталон помогает настроить профиль, провести первую заявку и разобраться с рабочим процессом в портале.'],
] as const;
...
export default function RoadFreightPage() {
  ...
  useEffect(() => {
    const titles: Array<[string, string]> = [
      ['road-benefits', 'Преимущества Каталон для перевозок'],
      ['road-market-stats', 'Ключевые показатели Каталон'],
      ['road-portal-preview', 'Как устроена работа в Каталон'],
      ...
      ['road-final-cta', 'Начните работать с перевозками в Каталон'],
    ];
    ...
    const safeDealSection = document.querySelector<HTMLElement>('[data-block-id="road-safe-deal-finance"]');
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#F7F6F3] text-[#19131F]">
      ...
      <main id="main-content">
          ...
          <div className="mx-auto max-w-[1440px] px-4 pb-16 pt-7 sm:px-6 lg:px-10 lg:pb-24">
            ...
            <div className="mt-10 grid items-center gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:gap-16">
              <div className="relative z-10">
                <div className="inline-flex items-center gap-2 rounded-full bg-white/50 px-4 py-2 text-xs font-bold text-[#440D84] shadow-sm backdrop-blur"><span className="h-2 w-2 animate-pulse rounded-full bg-[#6BCB00]" />Активное решение Каталон</div>
        ...
        <section id="benefits" className="border-b border-[#DED9E3] bg-white" aria-label="Преимущества Каталон">
        ...
        <section className="bg-[#F1F1ED] py-20 lg:py-24" id="portal-early"><div className="mx-auto max-w-[1208px] px-4 sm:px-6 lg:px-0"><div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><SectionIntro eyebrow="Продукт сразу" title="Посмотрите, как устроена работа в Каталон" text="Свободные грузы, карточка заявки, статусы сделки и документы собраны в одном рабочем интерфейсе." /></div><div className="mt-10 grid items-center gap-8 lg:grid-cols-[1.65fr_0.8fr]"><AssetPlaceholder name="portal-overview.png" /><div className="grid gap-3">{[[PackageCheck, 'Свободные грузы', 'Подбор заявок по маршруту и параметрам транспорта.'], [ClipboardCheck, 'Карточка заявки', 'Маршрут, стоимость, сроки и участники в одном окне.'], [Radar, 'Статусы сделки', 'Актуальный этап виден заказчику, перевозчику и экспедитору.'], [FileText, 'Документы', 'История и закрывающие файлы не теряются в чатах.']].map(([Icon, title, text]) => { const PortalIcon = Icon as LucideIcon; return <div key={title as string} className="group border-b border-[#DDD8DF] py-3"><div className="flex items-center gap-3 text-sm font-bold"><PortalIcon className="h-5 w-5 text-[#440D84] transition-transform group-hover:translate-x-1" />{title as string}</div><p className="mt-1 pl-8 text-xs leading-5 text-[#675F6F]">{text as string}</p></div>; })}</div></div></div></section>

        <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-10 lg:py-32" id="how-it-works" data-block-id="road-how-it-works" data-block-title="Как проходит перевозка в Каталон">
          <SectionIntro eyebrow="Один рынок — разные задачи" title="Каждый участник получает свой рабочий сценарий" text="Заказчик, перевозчик и экспедитор работают со своими задачами, но остаются внутри общей цифровой сделки." />
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3"><div className="rounded-2xl border border-[#DED9E3] bg-white/70 p-5 text-sm"><strong>Заказчику:</strong> предложения, тарифы, сроки и документы в одном окне.</div><div className="rounded-2xl border border-[#DED9E3] bg-white/70 p-5 text-sm"><strong>Перевозчику:</strong> прямые заказы, планирование загрузки и электронный документооборот.</div><div className="rounded-2xl border border-[#DED9E3] bg-white/70 p-5 text-sm"><strong>Экспедитору:</strong> контроль отклонений, участников и отчётности.</div></div>
        </section>

        <section className="bg-[#440D84] py-20 text-white lg:py-32" id="capabilities" data-block-id="road-capabilities" data-block-title="Инструменты Каталон как единая система">
        ...
        <section id="safe-deal" className="mx-auto max-w-[1440px] space-y-6 bg-white px-4 pb-10 pt-20 sm:px-6 lg:px-10 lg:pb-16 lg:pt-32" data-block-id="road-safe-deal-finance" data-block-title="Безопасная сделка и финансирование рейса">
        ...
        <section className="bg-white py-20 lg:py-32" data-block-id="road-earn-save" data-block-title="Зарабатывать больше и экономить на рейсе">
          <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10"><SectionIntro eyebrow="Экономика рейса" title="Помогаем больше зарабатывать и меньше тратить" text="Доход формируется не только ценой перевозки. Каталон помогает сократить простой, быстрее запустить рейс и подключить полезные сервисы в нужный момент." />
        ...
        <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-10 lg:py-32" id="portal" data-block-id="road-portal" data-block-title="Цифровой портал Каталон">
          <SectionIntro eyebrow="Портал Каталон" title="Управление перевозками — онлайн" text="Каждый раздел отвечает на конкретный рабочий вопрос. Интерфейс не заставляет собирать картину рейса вручную." />
        ...
        <section className="hidden bg-[#F1F1ED] py-20 lg:py-28" data-block-id="road-formats-hidden" data-block-title="Форматы перевозок — Для регулярных и разовых перевозок по России">
          <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
            ...
            <p className="mt-6 border-l-2 border-[#7133D0] pl-4 text-sm leading-6 text-[#675F6F]">Для опасных, негабаритных и специальных грузов Каталон развивает отдельные решения.</p>
          </div>
        </section>
        ...
        <section className="relative overflow-hidden bg-[#440D84] py-20 text-white lg:py-32" id="intelligence" data-block-id="road-ai-operator" data-block-title="Интеллектуальный помощник Каталон">
          ...
          <div className="relative mx-auto grid max-w-[1440px] gap-14 px-4 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:px-10">
            <div>
              <div className="inline-flex items-center gap-2 border border-[#B7FF2A]/35 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-[#B7FF2A]"><BrainCircuit className="h-4 w-4" />Интеллектуальный помощник</div>
              <h2 className="mt-7 text-[clamp(2.5rem,5vw,5.4rem)] font-extrabold leading-[0.94] tracking-[-0.055em]">Каталон берёт на себя <span className="text-[#B7FF2A]">операционную рутину</span></h2>
